...
class IntentClassifier:
    ...
    def __init__(self):
        self.emergency_keywords = [
            "emergency", "urgent", "help now", "crisis", "000", 
            "police", "ambulance", "fire", "suicide", "danger",
            "domestic violence", "assault", "hurt", "bleeding"
        ]
        
        self.exploitation_keywords = [
            "exploitation", "underpaid", "wage theft", "unfair", 
            "boss", "unsafe work", "employer", "not paid", "rights"
        ]
        
        self.digital_keywords = [
            "mygov", "online", "computer", "internet", "email", 
            "digital", "website", "centrelink", "app", "phone"
        ]
        
        self.economic_keywords = [
            "job", "work", "employment", "career", "skill", 
            "qualification", "business", "entrepreneur", "training"
        ]
        
        self.housing_keywords = [
            "house", "housing", "rent", "accommodation", "homeless", 
            "shelter", "eviction", "tenant", "lease"
        ]
...
class EmergencyHandler:
    ...
    def _generate_call_scripts(self, services: List[Dict], language: str) -> List[str]:
        ...
        scripts.extend([
            ...
            "My location is [your address]",
            "I am a refugee/migrant and need assistance"
        ])
...
class ResponseFormatter:
    ...
    async def format(self, services: List[Dict], suggestions: List[Dict], 
        ...
        # Create call scripts
        call_scripts = self._create_call_scripts(
            services=formatted_services[:3],
            language=language
        )
    ...
    def _generate_message(self, intent: Dict, services_count: int, 
                         urgency: str, language: str) -> str:
    ...
    def _generate_quick_replies(self, intent: Dict, suggestions: List[Dict]) -> List[str]:
        ...
        template = self.quick_reply_templates.get(
            intent["type"], 
            ["Tell me more", "Other services", "Emergency help", "Start over"]
        )
        ...
        "cards": [
            {
                ...
                "buttons": [
                    {"label": f"Call {service['phone']}", "value": service["phone"]}
                ] if service.get("phone") else []
            }
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting orchestrated API v2 on port {port}")
