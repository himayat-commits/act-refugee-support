...
if not USE_LIGHTWEIGHT:
    from src.search.economic import EconomicIntegrationSearch
else:
...
async def voiceflow_endpoint(request: Dict, authenticated: bool = Depends(verify_api_key)):
    ...
    query = ChatQuery(message=query_text, user_id=user_id, limit=3)
...
async def chat_endpoint(request: Dict, authenticated: bool = Depends(verify_api_key)):
...
async def search_resources(query: ChatQuery, authenticated: bool = Depends(verify_api_key)):
    ...
    try:
        ...
        if not results and any(
            word in query.message.lower() for word in ["job", "work", "skill", "qualification", "business"]
        ):
...
async def search_economic_integration(query: ChatQuery, authenticated: bool = Depends(verify_api_key)):
...
async def search_digital_support(query: ChatQuery, authenticated: bool = Depends(verify_api_key)):
...
async def search_exploitation_help(query: ChatQuery, authenticated: bool = Depends(verify_api_key)):
    ...
    for result in results:
        ...
        if isinstance(result, dict):
            formatted = {
                ...
                "services": (
                    result.get("services", "Multiple services")[:100] + "..."
                    if len(result.get("services", "")) > 100
                    else result.get("services", "Multiple services")
                ),
...
def detect_intent(message: str) -> str:
    ...
    # Emergency intents
    if any(
        word in message_lower for word in ["emergency", "urgent", "help now", "crisis", "000", "police", "ambulance"]
    ):
        return "emergency"

    # Digital/online help
    if any(
        word in message_lower for word in ["mygov", "online", "computer", "internet", "email", "digital", "website"]
    ):
        return "digital_help"

    # Exploitation/rights
    if any(
        word in message_lower for word in ["exploitation", "underpaid", "wage theft", "unfair", "boss", "unsafe work"]
    ):
        return "exploitation"
