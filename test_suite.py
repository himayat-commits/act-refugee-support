...
from models import RefugeeResource, SearchQuery, ResourceCategory
from config_light import QdrantConfig, OpenAIEmbedding
...
class TestAPIEndpoints:
    ...
    def test_economic_search(self):
        ...
        response = client.post("/search/economic", json={
            "message": "I need job training",
            "limit": 5
        })
        assert response.status_code == 200
        data = response.json()
...
class TestModels:
    ...
    def test_refugee_resource_validation(self):
        """Test RefugeeResource model validation"""
        valid_resource = {
            "name": "Test Resource",
        ...
        resource = RefugeeResource(**valid_resource)
    ...
    def test_search_query_validation(self):
        ...
        query = SearchQuery(
            query="test query",
            categories=[ResourceCategory.HEALTHCARE],
            urgency="high",
            limit=5
        )
