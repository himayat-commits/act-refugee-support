...
def demo_search_scenarios():
    ...
    print("-" * 40)
    query = SearchQuery(query="I need urgent help emergency medical doctor", urgency="high", limit=5)
    ...
    print("-" * 40)
    query = SearchQuery(query="I just arrived in Australia need help with housing English classes and job", limit=8)
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
    ...
    print("-" * 40)
    query = SearchQuery(
        query="visa application lawyer immigration appeal citizenship", categories=[ResourceCategory.LEGAL_AID], limit=5
    )
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
    ...
    print("-" * 40)
    query = SearchQuery(query="school children education homework English support kids", limit=6)
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
        print(f"  Category: {result.resource.category.value}")
    ...
    print("-" * 40)
    query = SearchQuery(
        query="job employment work resume interview career", categories=[ResourceCategory.EMPLOYMENT], limit=5
    )
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
    ...
    print("-" * 40)
    query = SearchQuery(query="women domestic violence safety health parenting mother", limit=5)
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
        print(f"  Category: {result.resource.category.value}")
    ...
    print("-" * 40)
    query = SearchQuery(query="overseas qualification skills assessment recognition professional degree", limit=5)
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
        print(f"  Services: {', '.join(result.resource.services_provided[:2])}...")
    ...
    print("-" * 40)
    query = SearchQuery(query="start business entrepreneur microfinance loan self-employment", limit=5)
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
        print(f"  Services: {', '.join(result.resource.services_provided[:2])}...")
    ...
    print("-" * 40)
    query = SearchQuery(query="professional career mentoring networking industry placement", limit=5)
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
        print(f"  Category: {result.resource.category.value}")
    ...
    print("-" * 40)
    query = SearchQuery(query="apprenticeship trade vocational certificate training TAFE", limit=5)
    results = search_engine.search(query)
    for result in results:
        print(f"• {result.resource.name}")
        print(f"  Cost: {result.resource.cost}")
...
def chatbot_integration_example():
    ...
    for user_input in user_queries:
        ...
        if results:
            ...
            print(
                "I couldn't find specific services for your query. Please call 131 450 for interpreter services or 000 for emergencies."
            )
