...
class DataPipeline:
    def __init__(self, config_light=None):
        """Initialize data pipeline with configuration"""
        self.config = config_light
        self.openai_client = OpenAI()
        self.qdrant_client = self.config.get_client() if config_light else None
    ...
    def calculate_quality_score(self, resource: Dict) -> float:
        ...
        if resource.get('description') and len(resource['description']) > 50: score += 1
        if resource.get('contact_phone'): score += 1
        if resource.get('contact_email'): score += 1
        if resource.get('contact_website'): score += 1
        if resource.get('contact_address'): score += 1
        if resource.get('services_provided'): score += 1
        if resource.get('languages_available'): score += 1
        if resource.get('cost'): score += 1
        if resource.get('verification_status') == 'verified': score += 1
    ...
    async def process_resources(self, resources: List[Dict], collection_name: str) -> Dict:
        ...
        for i, (resource, embedding) in enumerate(zip(enriched_resources, embeddings)):
            point = PointStruct(
                id=self.generate_resource_id(resource),
                vector=embedding,
                payload=resource
            )
            points.append(point)
