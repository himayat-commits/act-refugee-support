...
class SmartSearchEngine:
    ...
    def smart_search(
        ...
        scored_results = self._calculate_match_scores(
            filtered_results, 
            query, 
            filters, 
            user_context
        ...
        return [
            {
                'id': hit.id,
                'score': hit.score,
                'resource': EnhancedRefugeeResource(**hit.payload)
            }
            for hit in results
        ]
    ...
    def _check_filters(
        ...
        if filters.open_weekends:
            if not (resource.opening_hours.saturday != "Closed" or 
                   resource.opening_hours.sunday != "Closed"):
                return False
        ...
        if filters.language_required:
            languages = (resource.language_support.primary_languages + 
                        list(resource.language_support.staff_languages.keys()))
    ...
    def _check_language_match(
        ...
        if (resource.language_support.phone_interpreter or 
            user_language in resource.language_support.onsite_interpreters):
            return True
        
        return False
    
    def _check_accessibility_match(
        self,
        resource: EnhancedRefugeeResource,
        user_context: Optional[Dict]
    ) -> bool:
